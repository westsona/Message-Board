...
$article_id = isset($_GET['article']) ? $_GET['article'] : 1 ; //判断文章标题id，如果不知道就默认id为1
$article_sql = "SELECT article_name FROM article WHERE id=$article_id"; //从数据库中查询留言，首先查看置顶留言，再根据id倒序查询留言
$article_result = mysqli_query($link, $article_sql); //将上一步对数据库的查询结果返回给$result
...
<div class="background">
    ...
    <div class="transbox">
        <br />
        <h3>文章标题：<?php echo $article_name ?></h3>
        <h3>留言板</h3>
...
$sql = "SELECT * FROM message WHERE is_reported=0 and article_id=  $article_id ORDER BY is_toped DESC,id DESC"; //从数据库中查询留言，首先查看置顶留言，再根据id倒序查询留言
...
while($row=mysqli_fetch_assoc($result)){
    echo "<p>{$row['order_index']}# {$row['username']} 于 {$row['time']}说：<br> {$row['content']}";
    // //显示删除功能
